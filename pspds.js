  ...
  async function listCloudFiles() {
    ...
    try {
        const resp = await gapi.client.drive.files.list({
            ...
            fields: 'files(id, name, thumbnailLink, description, properties)',
                ...
                gallery.innerHTML += `
                    <div class='st-card'>
                        <div class='st-card-img-box'>
                            <img src='${file.thumbnailLink.replace('s220', 's500')}' 
     onclick='openFullPreview("${file.thumbnailLink.replace("s220", "s1000")}", "${name}")' 
     ...
     title='Click to view large image'/>
                        ...
                        <div class='st-card-actions'>
                            <!-- এখানে পরিবর্তন করা হয়েছে: সরাসরি লিঙ্কের বদলে ফাংশন কল হবে -->
                            <button onclick='deleteCloudFile("${file.id}", this)' class='st-btn-action btn-del'>
                                <i class="fa-solid fa-trash"></i> Delete
                            </button>
                            <button onclick='downloadDriveFile("${file.id}", "${file.name}")' class='st-btn-action btn-dl'>
        ...
        gallery.innerHTML = '<p style="color:red; text-align:center; grid-column:1/-1;">Connection error.</p>';
    }
}
...
async function downloadDriveFile(fileId, fileName) {
    try {
        // গুগলের এক্সেস টোকেন নেওয়া
        const accessToken = gapi.client.getToken().access_token;

        // ফাইলটি ব্লব (Blob) হিসেবে ডাউনলোড করার রিকোয়েস্ট
        ...
        // একটি অদৃশ্য লিঙ্ক তৈরি করে ডাউনলোড ট্রিগার করা
        const a = document.createElement('a');
        a.style.display = 'none';
        a.href = url;
        a.download = fileName; // ফাইলের নাম সেট করা
        ...
        console.error(err);
        triggerAlert("Error: Failed to download the image. Please try again.");
    }
}

function openFullPreview(imgUrl, customerName) {
    ...
    if (overlay && fullImg) {
        fullImg.src = imgUrl;
        caption.innerText = "Customer: " + customerName;
